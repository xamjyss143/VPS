...
# Function 1: Enable root login, password authentication, and set root password
enable_root_login() {
  local IP=$1
  local PORT=$2
  local USER=$3
...
# Function 3: Ensure root password is set to NEW_PASSWORD
set_root_password() {
  local IP=$1
  local PORT=$2
