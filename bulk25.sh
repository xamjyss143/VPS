...
while IFS= read -r line || [[ -n "$line" ]]; do
  ...
  if [[ "$PORT" != "22" ]]; then
    ...
    expect {
        ...
        "*\$ " { 
            send "echo '$OLD_PASS' | sudo -S sed -i '/^Port /d' /etc/ssh/sshd_config\r"
            send "echo '$OLD_PASS' | sudo -S bash -c \"echo 'Port 22' >> /etc/ssh/sshd_config\"\r"
            send "echo '$OLD_PASS' | sudo -S systemctl restart sshd\r"
            send "echo '$OLD_PASS' | sudo -S service ssh restart\r"
            send "exit\r"
        }
    }
    expect eof
EOF
  ...
  # Step 2: Enable root login & change password
  /usr/bin/expect <<EOF > /dev/null 2>&1
  spawn ssh -o StrictHostKeyChecking=no -p $PORT $USER@$IP
  expect {
      "*password:" { send "$OLD_PASS\r"; exp_continue }
      "*\$ " {
          send "echo '$OLD_PASS' | sudo -S sed -i '/^PermitRootLogin/d' /etc/ssh/sshd_config\r"
          send "echo '$OLD_PASS' | sudo -S sed -i '/^PasswordAuthentication/d' /etc/ssh/sshd_config\r"
          send "echo '$OLD_PASS' | sudo -S bash -c \"echo 'PermitRootLogin yes' >> /etc/ssh/sshd_config\"\r"
          send "echo '$OLD_PASS' | sudo -S bash -c \"echo 'PasswordAuthentication yes' >> /etc/ssh/sshd_config\"\r"
          send "echo '$OLD_PASS' | sudo -S systemctl restart sshd\r"
          send "echo '$OLD_PASS' | sudo -S service ssh restart\r"
